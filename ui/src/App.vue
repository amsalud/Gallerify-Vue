<template>
  <div id="app">
    <v-app style="background:#E3E3EE">
      <!-- Side Navbar -->
      <v-navigation-drawer app temporary fixed v-model="sideNavVisible">
        ...
        <v-list>
          ...
          <!-- Signout Button-->
          <v-list-tile ripple v-if="user" @click="signoutUser">
            <v-list-tile-action>
      ...
      <main>
        <v-container class="mt-4">
          ...
          <v-snackbar v-model="authSnackbar" color="success" :timeout="5000" top right>
           ...
           <!-- Auth Error Snackbar -->
          <v-snackbar v-if="authError" v-model="authErrorSnackbar" color="warning" :timeout="5000" top right>
            <v-icon class="mr-3">
              cancel
            </v-icon>
            <h3>{{authError.message}}</h3>
            <v-btn dark to="/signin">Sign in</v-btn>
          </v-snackbar>
        </v-container>
      </main>
    </v-app>
